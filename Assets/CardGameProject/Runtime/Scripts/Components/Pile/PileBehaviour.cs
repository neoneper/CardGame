...
namespace CardGameProject
{
    ...
    public abstract class PileBehaviour : MonoBehaviour
    {
        ...
        public RectTransform pilePanel => _pilePanel;
        public float spaceNodeAmount
        {
            get
            {
                return _nodeSpacingAmount;
            }
            set
            {
                _nodeSpacingAmount = value;
            }
        }
        /// <summary>
        ...
        /// </para>
        /// </summary>
        private List<IPileNode> _nodeObjects = new List<IPileNode>();
        ...
        private void RefreshCaches()
        {
            ...
            if (changedNodes > 0)
            {
                ...
                for (int i = 0; i < changedNodes; i++)
                {
                    ...
                    GameObject go = Instantiate(_visualNodePrefab, _pilePanel);
                    
                    if (go.TryGetComponent<IPileNode>(out IPileNode t))
                    {
                        _nodeObjects.Add(t);
                        t.Setup(this);
                    }
                    ...
                    go.name = $"{_pilePanel.name}_node_{_nodeObjects.Count.ToString()}";
                    EvalueNode(_nodeObjects.Count - 1);
                    OnNodeAdded(_nodeObjects.Count - 1);
                }
                return;
            }
            ...
            if (changedNodes < 0)
            {
                ...
                for (int i = 0; i < changedNodes; i++)
                {
                    ...
                    IPileNode nodeObj = GetNodeObject(index);
        ...
        private void EvalueNode(int index)
        {
            ...
            if (index == 0 && nodes == 1)
            {
                _cachedPositions[0] = settings.OriginOffset;
                _cachedRotations[0] = Quaternion.Euler(0, 0, settings.RotationOffset);
                _nodeObjects[0].GetGameObject().transform.localPosition = _cachedPositions[0];
                _nodeObjects[0].GetGameObject().transform.localRotation = _cachedRotations[0];
                return;
            }
            ...
            _cachedPositions[index] += _nodeObjects[index].OffsetPosition;
            ...
            _nodeObjects[index].GetGameObject().transform.localPosition = _cachedPositions[index];
            _nodeObjects[index].GetGameObject().transform.localRotation = _cachedRotations[index];
        ...
        public IPileNode GetNodeObject(int index)
        {
            if (!IsValidIndex(index)) { return null; }
            return _nodeObjects[index];
        }
        public IPileNode GetLastNodeObject()
        {
            if (!IsValidIndex(nodes - 1)) { return null; }
            return _nodeObjects[nodes - 1];
        }
        ...
        {
            if (!IsValidIndex(index)) { return default; }
            return GetNodeObject(index).OffsetPosition;
        }
        /// <summary>
        ...
        public void SetNodePositionOffset(int index, Vector2 offs)
        {
            ...
            GetNodeObject(index).OffsetPosition = offs;
        }
